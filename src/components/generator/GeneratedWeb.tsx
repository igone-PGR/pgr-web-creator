...
import { DEFAULT_CONTENT, DEFAULT_DESIGN } from "@/types/web-content";
...
const FONT_MAP: Record<string, string> = {
  ...
  "Clash Display": "Space+Grotesk:wght@400;500;600;700", // fallback
  "Cabinet Grotesk": "Space+Grotesk:wght@400;500;600;700", // fallback
...
const GeneratedWeb = ({ data, onBack }: GeneratedWebProps) => {
  ...
  const accentBg = design.accentGradient || scheme.primary;
          ...
          <section className="flex flex-col md:flex-row min-h-[65vh]">
            <div className="flex-1 flex flex-col justify-center px-8 md:px-16 py-16">
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                className="text-xs font-bold uppercase tracking-[0.3em] mb-6" style={{ color: scheme.primary, fontFamily: bodyFont }}>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-10">
                <a href="#contact" className="inline-flex items-center gap-2 px-8 py-4 text-sm font-bold text-white transition-all hover:scale-105"
                  style={{ background: isGradient ? accentBg : scheme.primary, borderRadius: radius }}>
      ...
      case "minimal-center":
        return (
          <section className="flex flex-col items-center justify-center text-center px-6 py-32 md:py-40" style={{ minHeight: heroH }}>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
              className="text-xs font-bold uppercase tracking-[0.3em] mb-8" style={{ color: scheme.primary, fontFamily: bodyFont }}>
              {project.slogan || project.sector}
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }} className="mt-12 flex gap-4">
              <a href="#contact" className="px-10 py-4 text-sm font-bold text-white transition-all hover:scale-105"
                style={{ background: isGradient ? accentBg : scheme.primary, borderRadius: radius }}>
                {content.heroCta}
              </a>
            </motion.div>
          ...
          <section className="max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-32">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              <div className="lg:col-span-7">
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                  className="text-xs font-bold uppercase tracking-[0.3em] mb-6" style={{ color: scheme.primary }}>{project.slogan || project.sector}</motion.p>
                <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.4 }}
                  contentEditable suppressContentEditableWarning
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-10 flex gap-4 items-center">
                  <a href="#contact" className="px-8 py-4 text-sm font-bold text-white transition-all hover:scale-105"
                    style={{ background: isGradient ? accentBg : scheme.primary, borderRadius: radius }}>
      ...
      case "gradient-overlay":
        return (
          <section className="relative overflow-hidden flex items-end" style={{ minHeight: heroH }}>
            <div className="absolute inset-0" style={{ background: isGradient ? accentBg : `linear-gradient(135deg, ${scheme.primary}, ${scheme.primary}cc)` }} />
            ...
            <div className="relative max-w-7xl mx-auto px-6 md:px-12 pb-20 md:pb-32 pt-32 w-full">
              <motion.h1 initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}
                contentEditable suppressContentEditableWarning
                className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[0.92] text-white max-w-4xl outline-none"
                style={{ fontFamily: headingFont }}>{content.heroHeadline}</motion.h1>
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
                contentEditable suppressContentEditableWarning
                className="mt-6 text-xl max-w-xl leading-relaxed text-white/80 outline-none" style={{ fontFamily: bodyFont }}>{content.heroSubtitle}</motion.p>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="mt-10">
                <a href="#contact" className="inline-flex items-center gap-2 px-8 py-4 text-sm font-bold transition-all hover:scale-105"
                  style={{ background: dark ? "#0a0a0f" : "#fff", color: scheme.primary, borderRadius: radius }}>
                  {content.heroCta} <ArrowUpRight className="w-4 h-4" />
                </a>
              </motion.div>
            </div>
          ...
          <section className="relative overflow-hidden">
            <div className="w-full relative" style={{ height: heroH }}>
              ...
              <div className="absolute inset-0" style={{
                background: dark
                  ? "linear-gradient(180deg, rgba(10,10,15,0.2) 0%, rgba(10,10,15,0.95) 100%)"
                  : "linear-gradient(180deg, rgba(250,250,249,0.05) 0%, rgba(250,250,249,0.97) 100%)",
              }} />
              <div className="absolute inset-0 flex flex-col justify-end max-w-7xl mx-auto px-6 md:px-12 pb-16 md:pb-24">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}
                  ...
                  <a href="#contact" className="px-8 py-4 text-sm font-bold text-white transition-all hover:scale-105 hover:shadow-xl"
                    style={{ background: isGradient ? accentBg : scheme.primary, borderRadius: radius }}>
        ...
        {content.features.map((feat, i) => {
          ...
          return (
            <motion.div key={i} {...getAnim(i)}
              ...
              {design.decorativeElements && (
                <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full opacity-10" style={{ background: scheme.primary }} />
              )}
              <div className="w-12 h-12 flex items-center justify-center mb-6 transition-transform group-hover:scale-110"
                style={{ backgroundColor: `${scheme.primary}15`, borderRadius: radiusSm }}>
  ...
  const renderAbout = () => (
    <section id="about" className="py-24 md:py-32" style={{ backgroundColor: bgAlt }}>
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <motion.p {...getAnim()}
          className="text-xs font-bold uppercase tracking-[0.3em] mb-8" style={{ color: scheme.primary, fontFamily: bodyFont }}>{content.aboutTitle}</motion.p>
        <motion.p {...getAnim(1)}
          contentEditable suppressContentEditableWarning
        ...
        return (
          <section className="py-8">
            {project.photos.map((photo, i) => (
              <motion.div key={i} {...getAnim(i)}
                className={`w-full ${i % 2 === 0 ? "" : "max-w-5xl mx-auto px-6"}`}>
                <div className={`overflow-hidden ${i % 2 === 0 ? "aspect-[21/9]" : "aspect-[16/9]"}`}
                  style={{ borderRadius: i % 2 === 0 ? "0" : radius }}>
                  <img src={photo} alt={`${project.businessName} ${i + 1}`}
      ...
      case "list":
        return (
          <section id="services" className="max-w-7xl mx-auto px-6 md:px-12 py-24 md:py-32">
            <motion.p {...getAnim()} className="text-xs font-bold uppercase tracking-[0.3em] mb-4" style={{ color: scheme.primary }}>{content.servicesTitle}</motion.p>
            <div className="mt-10 divide-y" style={{ borderColor: border }}>
              {content.services.map((service, i) => (
                <motion.div key={i} {...getAnim(i)} className="group py-8 flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-default">
                  <div className="flex items-center gap-6">
                    <span className="text-5xl font-black tracking-tighter opacity-20" style={{ color: scheme.primary, fontFamily: headingFont }}>
            ...
            <div className="mt-12 space-y-16">
              {content.services.map((service, i) => (
                <motion.div key={i} {...getAnim(i)} className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                  <span className="md:col-span-2 text-8xl font-black tracking-tighter" style={{ color: `${scheme.primary}20`, fontFamily: headingFont }}>
                    {String(i + 1).padStart(2, "0")}
                  </span>
      ...
      case "timeline":
        return (
          <section id="services" className="max-w-4xl mx-auto px-6 md:px-12 py-24 md:py-32">
            <motion.p {...getAnim()} className="text-xs font-bold uppercase tracking-[0.3em] mb-12 text-center" style={{ color: scheme.primary }}>{content.servicesTitle}</motion.p>
            <div className="relative">
              <div className="absolute left-6 top-0 bottom-0 w-px" style={{ backgroundColor: `${scheme.primary}30` }} />
              {content.services.map((service, i) => (
                <motion.div key={i} {...getAnim(i)} className="relative pl-16 pb-12 last:pb-0">
                  <div className="absolute left-4 top-1 w-5 h-5 rounded-full border-2" style={{ borderColor: scheme.primary, backgroundColor: card }} />
        ...
        return (
          <section id="services" className="max-w-7xl mx-auto px-6 md:px-12 py-24 md:py-32">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {content.services.map((service, i) => (
                <motion.div key={i} {...getAnim(i)}
                  className="group p-8 transition-all hover:shadow-lg cursor-default"
                  style={{ border: `1px solid ${border}`, backgroundColor: card, borderRadius: radius }}>
                  <div className="flex items-start justify-between mb-5">
                    <span className="text-4xl font-black tracking-tighter" style={{ color: `${scheme.primary}30`, fontFamily: headingFont }}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
    ...
    <section id="contact" className="py-24 md:py-32" style={{ backgroundColor: bgAlt }}>
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div>
            <motion.p {...getAnim()} className="text-xs font-bold uppercase tracking-[0.3em] mb-4" style={{ color: scheme.primary }}>{content.contactTitle}</motion.p>
            ...
            <div className="space-y-5 mt-10">
              {(project.businessEmail || project.email) && (
                <a href={`mailto:${project.businessEmail || project.email}`} className="flex items-center gap-4 group">
                  <div className="w-10 h-10 flex items-center justify-center" style={{ backgroundColor: `${scheme.primary}15`, borderRadius: radiusSm }}>
          ...
          <motion.div {...getAnim(2)}
            className="p-8 md:p-10 border flex flex-col items-center justify-center text-center gap-6"
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              {(project.businessEmail || project.email) && (
                <a href={`mailto:${project.businessEmail || project.email}`}
                  ...
                  style={{ backgroundColor: `${scheme.primary}15`, color: scheme.primary, borderRadius: radius }}>
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: bg, color: text1, fontFamily: bodyFont }}>
      ...
      <nav className="max-w-7xl mx-auto px-6 md:px-12 py-6 flex items-center justify-between">
        ...
        <a href={`mailto:${project.businessEmail || project.email}`}
          className="hidden md:flex items-center gap-1.5 text-xs font-semibold px-5 py-2.5 transition-all hover:scale-105 text-white"
          style={{ background: isGradient ? accentBg : scheme.primary, borderRadius: radius }}>
