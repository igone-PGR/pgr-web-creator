...
serve(async (req) => {
  ...
  try {
    ...
    // Photo instructions - images are already resolved URLs (client or AI-generated)
    let photoInstructions: string;
    if (input.photoUrls.length > 0) {
      photoInstructions = `FOTOS: Reemplaza los atributos src="" de las etiquetas <img> con estas URLs en orden:\n${input.photoUrls.map((url, i) => `  Foto ${i + 1}: ${url}`).join("\n")}\nSi hay más imágenes en la plantilla que fotos proporcionadas, reutiliza las fotos cíclicamente.\nNUNCA inventes URLs de imagen.`;
    } else {
      photoInstructions = `NO HAY FOTOS: Mantén TODAS las URLs de imágenes existentes en la plantilla EXACTAMENTE como están. NO modifiques ningún atributo src de imágenes.`;
    }

    const logoInstruction = input.logoUrl
      ? `LOGO: Donde haya un logo o nombre de marca como imagen, usa esta URL: ${input.logoUrl}`
