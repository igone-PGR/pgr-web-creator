...
serve(async (req) => {
  ...
  try {
    ...
    const langMap: Record<string, string> = {
      es: "español", en: "inglés (English)", fr: "francés (Français)",
    };
...
- Crea una paleta de colores COMPLETA basada en la personalidad del negocio.
- Un bar nocturno → tonos oscuros y neones. Una floristería → cálidos naturales. Un restaurante → colores apetitosos.
...
2. Una paleta de colores ÚNICA con: bg, bgAlt, card, text1, text2, border, accent, accentText, accentDark.
   - accent: color principal de marca
   - accentDark: versión más oscura del accent (para fondos hero/CTA)
   - Asegura buen contraste entre accentText y accent/accentDark.`;
            ...
            function: {
              ...
              parameters: {
                ...
                properties: {
                  ...
                  heroCta: { type: "string" },
                  heroStats: { type: "array", items: { type: "object", properties: { value: { type: "string" }, label: { type: "string" } }, required: ["value", "label"] } },
                  categories: { type: "array", items: { type: "object", properties: { title: { type: "string" }, icon: { type: "string" } }, required: ["title", "icon"] } },
                  aboutTitle: { type: "string" },
                  aboutText: { type: "string" },
                  aboutHighlights: { type: "array", items: { type: "object", properties: { title: { type: "string" }, description: { type: "string" } }, required: ["title", "description"] } },
                  servicesTitle: { type: "string" },
                  servicesSubtitle: { type: "string" },
                  services: { type: "array", items: { type: "object", properties: { name: { type: "string" }, description: { type: "string" } }, required: ["name", "description"] } },
                  ctaTitle: { type: "string" },
                  ctaSubtitle: { type: "string" },
                  ctaCta: { type: "string" },
                  ctaStats: { type: "array", items: { type: "object", properties: { value: { type: "string" }, label: { type: "string" } }, required: ["value", "label"] } },
                  ...
                  testimonials: { type: "array", items: { type: "object", properties: { name: { type: "string" }, text: { type: "string" }, rating: { type: "number" } }, required: ["name", "text", "rating"] } },
                  ...
                  colors: {
                    type: "object",
                    properties: {
                      bg: { type: "string" }, bgAlt: { type: "string" }, card: { type: "string" },
                      text1: { type: "string" }, text2: { type: "string" }, border: { type: "string" },
                      accent: { type: "string" }, accentText: { type: "string" }, accentDark: { type: "string" },
                    },
                    required: ["bg", "bgAlt", "card", "text1", "text2", "border", "accent", "accentText", "accentDark"],
                  },
                },
                required: [
                  "heroHeadline", "heroSubtitle", "heroCta", "heroStats",
                  "categories", "aboutTitle", "aboutText", "aboutHighlights",
                  "servicesTitle", "servicesSubtitle", "services",
                  "ctaTitle", "ctaSubtitle", "ctaCta", "ctaStats",
                  "featuresTitle", "featuresSubtitle", "features",
                  "testimonials", "faq",
                  "contactTitle", "contactSubtitle", "footerTagline", "colors",
                ],
              },
            },
          },
    ...
    if (!response.ok) {
      ...
      if (status === 429) {
        return new Response(JSON.stringify({ error: "Demasiadas solicitudes. Inténtalo de nuevo." }), { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      if (status === 402) {
        return new Response(JSON.stringify({ error: "Créditos agotados. Contacta con soporte." }), { status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      console.error("AI gateway error:", status);
      return new Response(JSON.stringify({ error: "Error al generar el contenido." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (e instanceof z.ZodError) {
      return new Response(JSON.stringify({ error: "Datos inválidos. Revisa los campos." }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
